...
cluster_res <- readRDS("./cluster_AML/AMLonly_6clus.rds")
...
for(i in unique(survdata$ICC)){
  ...
  colours_clusters <- c("blue", plasma(5))
  ...
  library("survminer")
  file<-paste0("../comparison/Supp_",gsub("MDS/AML ", "MDSAML_",i),"AML_only.png")
  ...
  p[i] <- ggsurvplot(
    ...
    pval.size =4,
    pval.coord = c(0, 0.15),
    title= paste0("OS - AML only -", gsub(pattern = "_", replacement = " ", i), " (n=",length(clinical$event)/2,")"),
...
pdf(file = "../analysis/figures/Supplementary_MDSAML_OS_AMLonly.pdf", width = 16, height = 10)
...
cluster_res <- readRDS("./AMLonly_6clus.rds")
#clusterMDS <- readRDS("./MDSonly_6clus.rds")
#readRDS("../results/euler_memberships_8k_9clusters.rds")
...
for(i in unique(survdata$ICC)){
  ...
  p[i] <- ggsurvplot(
    ...
    risk.table.fontsize = 3,
    pval.size =4,
    title= paste0("AMLt - AML only - ", gsub(pattern = "_", replacement = " ", i), " (n=",length(clinical$event)/2,")"),
    risk.table.y.text = FALSE # show bars instead of names in text annotations
    # in legend of risk table
...
pdf(file = "../analysis/figures/Supplementary_MDSAML_AMLt_AMLonly.pdf", width = 16, height = 10)
ggarrange(p$`MDS/AML MR-Cyto`,p$`MDS/AML MR-Gen`, p$`MDS/AML TP53`, p$`MDS/AML NOS`, ncol = 2, nrow = 2 , labels = letters[1:4])
dev.off() 
