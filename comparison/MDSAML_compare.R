...
for(i in unique(survdata$ICC)){
  ...
  mypal=colours_clusters[c(1, c(seq(1:9)+1)[LETTERS[1:9] %in% unique(clinical$group)[grep(x = unique(clinical$group), pattern = i, invert = T)]])]
...
pdf(file = "../analysis/figures/Supplementary_MDSAML_OS.pdf", width = 11, height = 10)
ggarrange(p$`MDS/AML MR-Cyto`,p$`MDS/AML MR-Gen`, p$`MDS/AML TP53`, p$`MDS/AML NOS`, ncol = 2, nrow = 2 , labels = letters[1:4])
dev.off()
...
pdf(file = "../analysis/figures/Supplementary_MDSAML_AMLt.pdf", width = 11, height = 10)
ggarrange(p$`MDS/AML MR-Cyto`,p$`MDS/AML MR-Gen`, p$`MDS/AML TP53`, p$`MDS/AML NOS`, ncol = 2, nrow = 2 , labels = letters[1:4])
dev.off() 
